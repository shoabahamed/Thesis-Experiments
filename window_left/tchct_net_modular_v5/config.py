...
# ──────────────────────────────────────────────────────────────────────
# Feature definition (138 raw Leap features, including elbow)
# ──────────────────────────────────────────────────────────────────────
HANDS = ["left", "right"]
...
# Elbow coordinates per hand.
for _hand in HANDS:
    for _axis in CARTESIAN_AXES:
        FEATURE_KEYS.append(f"{_hand}_elbow_{_axis}")
...
assert len(FEATURE_KEYS) == 138, f"Expected 138 features, got {len(FEATURE_KEYS)}"
...
for _hand in HANDS:
    ...
    # Elbow: included so rotation/normalization applies to it
    _triplets.append(
        tuple(FEATURE_INDEX[f"{_hand}_elbow_{axis}"] for axis in CARTESIAN_AXES)
    )
    for _finger in FINGERS:
        for _bone in BONES:
            _triplets.append(
