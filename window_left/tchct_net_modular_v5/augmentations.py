...
class SignLanguageAugmentationPipeline:
    ...
    def __call__(self, sequence: np.ndarray) -> np.ndarray:
        """Apply composed augmentations to the given sequence (T, 138)."""
        if sequence.ndim != 2 or sequence.shape[1] != 138:
            raise ValueError(f"Expected sequence with shape (T, 138), got {sequence.shape}")
