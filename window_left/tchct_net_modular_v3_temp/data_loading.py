...
from disambiguation import RAW_AUX_KEYS
...
def extract_segments_for_recording(
    ...
    aux_cols = [col for col in RAW_AUX_KEYS if col in raw_df.columns]
    if aux_cols:
        aux_partial = raw_df.loc[:, aux_cols].to_numpy(dtype=np.float32)
        aux_features = np.zeros((features.shape[0], len(RAW_AUX_KEYS)), dtype=np.float32)
        for i, col in enumerate(aux_cols):
            aux_features[:, RAW_AUX_KEYS.index(col)] = aux_partial[:, i]
    else:
        aux_features = np.zeros((features.shape[0], len(RAW_AUX_KEYS)), dtype=np.float32)
    ...
    for seg in interval_defs:
        ...
        records.append({
            ...
            "segment_aux": aux_features[start : end + 1],
            "recording_aux": aux_features,
        })
