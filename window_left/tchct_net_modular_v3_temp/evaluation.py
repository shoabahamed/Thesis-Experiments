...
def _disambig_overrides(tau_margin: float | None, lam: float | None, top_k: int | None) -> dict:
    """Build a kwargs dict of only the Hook B knobs that were explicitly
    overridden, so stream_model_online's own config-sourced defaults are
    used for anything left as None (e.g. during calibration sweeps)."""
    overrides = {}
    if tau_margin is not None:
        overrides["tau_margin"] = tau_margin
    if lam is not None:
        overrides["lam"] = lam
...
def evaluate_streaming_metrics(
    ...
    for idx, sample in enumerate(samples, start=1):
        # Run decoder
        stream_steps, emitted_preds, emit_regions, _diag = stream_model_online(
            V=sample["V"],
            model_obj=model_obj,
            ...
            **_disambig_overrides(tau_margin, lam, top_k),
        )
...
def evaluate_model_wer(
    ...
    for idx, sample in enumerate(samples, start=1):
        stream_steps, final_preds, emit_regions, diagnostics = stream_model_online(
            V=sample["V"],
            model_obj=model_obj,
            normalize_fn=normalize_fn,
            id_to_label=id_to_label,
        ...
        rows.append({
            ...
            "hook_a_rescue_count":        diagnostics["hook_a_rescue_count"],
            "hook_b_triggered_count":     diagnostics["hook_b_triggered_count"],
            "hook_b_total_emissions":     diagnostics["hook_b_total_emissions"],
            "hook_b_label_changed_count": diagnostics["hook_b_label_changed_count"],
        })
