...
def build_dataloaders(
    ...
    train_sampler = GlossBalancedBatchSampler(
        labels=y_train,
        glosses_per_batch=GLOSS_BALANCED_GLOSSES_PER_BATCH,
        samples_per_gloss=GLOSS_BALANCED_SAMPLES_PER_GLOSS,
        seed=SEED,
    )
...
def run_single_fold(
    ...
    # Save a dedicated test-split confusion matrix with row normalization and
    # off-diagonal misclassification counts.
    test_acc, test_preds, test_targets = evaluate_model_full(
        model_obj=model,
        loader=test_loader,
        num_classes=num_classes,
        split_name=f"Test ({test_user})",
    )
    class_names = [split_data["id_to_label"][i] for i in range(num_classes)]
    val_acc, val_preds, val_targets = evaluate_model_full(
        model_obj=model,
        loader=val_loader,
        num_classes=num_classes,
        split_name="Val",
