...
# Keep decoder defaults aligned with the notebook online-WER section.
DECODER_MAJOR_VOTE_B = 20
DECODER_SIGN_CONF_THRESHOLD = 0.8
DECODER_SIGN_BG_MARGIN = 0.1
...
def load_stgcn_runtime_checkpoint(path: str) -> dict:
    ...
    return {
        ...
        "majority_vote_B": max(1, int(config.get("majority_vote_B", metadata.get("majority_vote_B", DECODER_MAJOR_VOTE_B)))),
        "sign_conf_threshold": float(config.get("sign_conf_threshold", metadata.get("sign_conf_threshold", DECODER_SIGN_CONF_THRESHOLD))),
        "sign_bg_margin": float(config.get("sign_bg_margin", metadata.get("sign_bg_margin", DECODER_SIGN_BG_MARGIN))),
        "background_label": str(config.get("background_label", BACKGROUND_LABEL)),
    }


class OnlineCausalDecoder:
    """Streaming decoder: confidence gate -> causal vote -> transition emission."""
    ...
    def __init__(
        ...
        self.vote_history = deque(maxlen=self.majority_vote_B)
        self.last_voted = None

        self.majority_vote_threshold = float(majority_vote_threshold)

        self.background_id = None
        for cls_id, cls_name in self.id_to_label.items():
            if cls_name == self.background_label:
                self.background_id = int(cls_id)
                break

    def update(self, probs: np.ndarray) -> dict:
        pred_id = int(np.argmax(probs))
        pred_label = str(self.id_to_label.get(pred_id, f"sign_{pred_id}"))
        pred_conf = float(probs[pred_id])

        bg_conf = 0.0
        if self.background_id is not None and self.background_id < probs.shape[0]:
            bg_conf = float(probs[self.background_id])

        if self.enable_confidence_gate:
            keep_sign = (
                pred_label != self.background_label
                and pred_conf >= self.sign_conf_threshold
                and (pred_conf - bg_conf) >= self.sign_bg_margin
            )
            gated_label = pred_label if keep_sign else self.background_label
        else:
            gated_label = pred_label

        self.vote_history.append(gated_label)
        # ---- NEW block ----
        most_common = Counter(self.vote_history).most_common(1)
        if most_common:
            top_label, top_count = most_common[0]
            total = len(self.vote_history)
            # Check if the majority vote exceeds the required threshold
            if (top_count / total) > self.majority_vote_threshold:
                voted_label = top_label
            else:
                voted_label = self.background_label
        else:
            # (vote_history empty, should not happen after appending)
            voted_label = self.background_label
        # ---- end NEW ----
        ...
        if voted_label != self.last_voted:
            ...
            if voted_label != self.background_label:
                emitted_label = voted_label
...
def inference_process(frame_queue: mp.Queue, buffer_size: int, stop_event, ready_event):
    ...
    print(
        ...
        f"B={runtime['majority_vote_B']}, "
        f"sign_thr={runtime['sign_conf_threshold']:.2f}, "
    ...
    decoder = OnlineCausalDecoder(
        ...
        majority_vote_B=runtime["majority_vote_B"],
        sign_conf_threshold=runtime["sign_conf_threshold"],
    ...
    while True:
        ...
        if len(buffer) >= MIN_BUFFER_FOR_PREDICTION:
            ...
            with torch.inference_mode():
                ...
                # Keep float32 casting inline directly without unnecessary detach overhead since inference_mode takes care of grads
            ...
            # Treat sustained background as end-of-sentence boundary.
            if background_streak >= runtime["majority_vote_B"] and sentence_tokens:
