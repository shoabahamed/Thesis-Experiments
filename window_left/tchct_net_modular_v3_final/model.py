...
from config import DEVICE, HANDS, HAND_POSITION_TRIPLETS, PALM_TRIPLETS
...
# config.FEATURE_KEYS interleaves both hands' palm/wrist first (12 values),
# then all of hand0's finger bones, then all of hand1's finger bones. This
# permutation gathers each hand's 22 slots (palm, wrist, 20 finger bones)
# into a contiguous 66-value block, in HANDS order, so the reshape below
# actually separates the two hands instead of scrambling them together.
...
assert len(_HAND_FEATURE_PERM) == NUM_FEATURES, (
    f"Expected {NUM_FEATURES} permuted feature indices, got {len(_HAND_FEATURE_PERM)}"
)
...
class TransformerStream(nn.Module):
    """
    Input  : (B, C=3, T=window_size, V=22, M=2)
...
class THCTNet(nn.Module):
    ...
    def forward(self, sequences: Tensor, lengths: Optional[Tensor] = None) -> Tensor:
        """
        sequences : (B, T=window_size, D=132)
