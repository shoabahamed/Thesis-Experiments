...
class FullSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if self.normalize_fn is not None:
            import inspect
            sig = inspect.signature(self.normalize_fn)
            if "user" in sig.parameters:
                video = self.normalize_fn(video, user=sample["user"])
            else:
...
class LeapSignDataset(Dataset):
    ...
    def __init__(self, X: np.ndarray, y: np.ndarray, users: list[str] | None = None, normalize_fn=None, augment_pipeline=None):
    ...
    def __getitem__(self, idx):
        ...
        user = self.users[idx] if self.users is not None else None
        if self.normalize_fn is not None:
            import inspect
            sig = inspect.signature(self.normalize_fn)
            if "user" in sig.parameters:
                sequence_np = self.normalize_fn(sequence_np, user=user)
            else:
