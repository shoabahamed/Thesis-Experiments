...
def build_dataloaders(
    ...
    X_train, y_train, users_train = build_windows_from_segments(
        split_data["train_segments"],
        split_data["label_to_id"],
        window_size=WINDOW_SIZE,
        stride=STRIDE,
    )
    X_val, y_val, users_val = build_windows_from_segments(
        split_data["val_segments"],
        split_data["label_to_id"],
        window_size=WINDOW_SIZE,
        stride=STRIDE,
    )
    X_test, y_test, users_test = build_windows_from_segments(
    ...
    if len(val_mask) > 0:
        ...
        users_val = [u for idx_mask, u in enumerate(users_val) if val_mask[idx_mask]]

    test_mask = np.array([lbl in known_labels for lbl in y_test], dtype=bool) if len(y_test) > 0 else np.array([], dtype=bool)
    if len(test_mask) > 0:
        X_test = X_test[test_mask]
        y_test = y_test[test_mask]
        users_test = [u for idx_mask, u in enumerate(users_test) if test_mask[idx_mask]]

    train_ds = LeapSignDataset(
        X_train, y_train, users=users_train, normalize_fn=NORMALIZE_FN, augment_pipeline=augment_pipeline
    )
    val_ds = LeapSignDataset(
        X_val, y_val, users=users_val, normalize_fn=NORMALIZE_FN
    )
    test_ds = LeapSignDataset(
        X_test, y_test, users=users_test, normalize_fn=NORMALIZE_FN
    )
