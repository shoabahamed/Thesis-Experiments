...
def random_rotate_3d(sequence: np.ndarray, max_degrees: float, rng: np.random.Generator) -> np.ndarray:
    ...
    One rotation is drawn per sample and applied identically to both hands:
    the physical causes being simulated (sensor-mount tilt, whole-body posture
    relative to the sensor) rotate the entire scene, so both hands must share
    the same R. A shared global rotation survives palm-relative normalization
    as that same R applied to each hand's normalized coordinates.
    """
