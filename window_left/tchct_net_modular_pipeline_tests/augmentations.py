...
def add_coordinate_noise(
    ...
    for hand in HANDS:
        ...
        noise = rng.normal(0.0, noise_std, size=(T, len(flat_idx))).astype(np.float32)
        out[:, flat_idx] += noise
...
def verify_augmented_sample(
    ...
    for hand in orig_bones:
        ...
        if mean_orig > 1e-5:
            ...
            # Maximum 25% relative change or 15mm absolute change threshold
            if np.any(rel_diff > 0.25) and np.any(diff > 15.0):
                return False
    ...
    if aug_max > orig_max * limit_factor + 50.0 or aug_min < orig_min * limit_factor - 50.0:
        return False
    ...
    if len(orig_diffs) > 0:
        ...
        if max_aug_diff > max_orig_diff * limit_factor + 50.0:
            return False
...
class SignLanguageAugmentationPipeline:
    ...
    def __init__(
        ...
        # Usage stats: how often each transform actually fires, and how often
        # the whole sample falls back to the original due to failed verification.
        self.stats = {
            "calls": 0,
            "dropout_applied": 0,
            "scaling_applied": 0,
    ...
    def stats_report(self) -> str:
        """Format applied-vs-defaulted percentages for the calls made so far."""
        calls = self.stats["calls"]
        if calls == 0:
            return "  (no augmentation calls recorded)"

        def pct(n: int) -> str:
            return f"{n}/{calls} ({100.0 * n / calls:.1f}%)"

        lines = [
            f"  Total calls           : {calls}",
            f"  Frame dropout applied : {pct(self.stats['dropout_applied'])}",
            f"  Scaling applied       : {pct(self.stats['scaling_applied'])}",
            f"  Rotation applied      : {pct(self.stats['rotation_applied'])}",
            f"  Noise applied         : {pct(self.stats['noise_applied'])}",
            f"  Fallback to original  : {pct(self.stats['fallback_to_original'])} "
            "(failed verification -> original window used)",
        ]
        return "\n".join(lines)
