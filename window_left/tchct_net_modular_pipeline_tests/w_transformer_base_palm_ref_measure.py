...
STGCN_CHECKPOINT_PATH = os.path.join(_MODELS_ROOT, "models_updated", "trained_models", "20260705T121022Z_thct_net_val-0.9959_be83adac_user5.pt")
SENTENCE_OUTPUT_PATH = os.path.join(_MODELS_ROOT, "temp", "live_inference_sentence.txt")

# ── Per-frame end-to-end latency measurement (for thesis reporting) ──
# Every frame is timestamped the instant it "arrives" in the main capture
# loop (t_capture). Each pipeline stage is timestamped downstream so the
# exact wall-clock cost of every stage, and the total capture->result
# latency, can be reconstructed per frame.
LATENCY_LOG_PATH = os.path.join(_MODELS_ROOT, "temp", "frame_latency_log.csv")
LATENCY_SUMMARY_PATH = os.path.join(_MODELS_ROOT, "temp", "frame_latency_summary.txt")
...
# inter-sign gap.
SENTENCE_BREAK_FRAMES = 300   # ~1.5s at 30 FPS
...
def inference_process(frame_queue: mp.Queue, buffer_size: int, stop_event, ready_event):
    ...
    # Per-frame latency records (thesis measurement) — flushed to CSV on shutdown.
    ...
    while True:
        ...
        frame_number, hand_data, cam_frame, t_capture = item
        _ = cam_frame
        frames_received += 1
        t_queue_recv = time.perf_counter()
        ...
        t_features_ready = time.perf_counter()

        newest_frame = buffer[-1]["frame_number"]
        has_prediction = len(buffer) >= MIN_BUFFER_FOR_PREDICTION
        ...
        if has_prediction:
            ...
            with torch.inference_mode():
                ...
                if device.type == "cuda":
                    torch.cuda.synchronize()  # ensure GPU work is actually finished before timestamping
            ...
            # Frame is now fully processed (prediction + decode + sentence update).
            # Timestamp BEFORE the console prints so print I/O is not counted.
            t_frame_done = time.perf_counter()
        ...
        # ── record end-to-end latency for this frame (thesis measurement) ──
        total_latency_ms = (t_frame_done - t_capture) * 1000.0
        queue_latency_ms = (t_queue_recv - t_capture) * 1000.0
        feature_latency_ms = (t_features_ready - t_queue_recv) * 1000.0
        inference_latency_ms = (
            (t_inference_done - t_features_ready) * 1000.0
            if t_inference_done is not None else None
        )
        decode_latency_ms = (
            (t_frame_done - t_inference_done) * 1000.0
            if t_inference_done is not None else None
        )
        ...
        print(f"[Latency] frame {frame_number:>5d} | total {total_latency_ms:7.2f} ms"
              + (f" | inference {inference_latency_ms:6.2f} ms" if inference_latency_ms is not None else ""))
    ...
    _write_latency_report(latency_records)


def _write_latency_report(latency_records: list[dict]) -> None:
    """Dump the raw per-frame latency log to CSV and a human-readable summary (mean/median/std/min/max/p95/p99)."""
    if not latency_records:
        print("[Latency] No frames were processed — nothing to report.")
        return

    os.makedirs(os.path.dirname(LATENCY_LOG_PATH), exist_ok=True)
    fieldnames = [
        "frame_number", "has_prediction", "queue_latency_ms", "feature_latency_ms",
        "inference_latency_ms", "decode_latency_ms", "total_latency_ms",
    ]
    ...
    total_values = [r["total_latency_ms"] for r in latency_records]
    queue_values = [r["queue_latency_ms"] for r in latency_records]
    feature_values = [r["feature_latency_ms"] for r in latency_records]
    inference_values = [r["inference_latency_ms"] for r in latency_records if r["inference_latency_ms"] is not None]
    decode_values = [r["decode_latency_ms"] for r in latency_records if r["decode_latency_ms"] is not None]

    sections = {
        "TOTAL (frame arrival -> fully processed, all frames)": _stats(total_values),
        "Queue wait (capture -> received by inference process)": _stats(queue_values),
        "Feature extraction + GPU normalization": _stats(feature_values),
        "Model inference (forward pass only, frames with a full buffer)": _stats(inference_values) if inference_values else None,
        "Decoding + bookkeeping (frames with a full buffer)": _stats(decode_values) if decode_values else None,
    }

    lines = []
    lines.append("Per-frame end-to-end latency report")
    lines.append(f"generated_at: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"total_frames_logged: {len(latency_records)}")
    lines.append("")
    for title, stats in sections.items():
        lines.append(title + ":")
        if stats is None:
            lines.append("  (no samples)")
        else:
            lines.append(
                f"  n={stats['n']}  mean={stats['mean']:.3f} ms  median={stats['median']:.3f} ms  "
                f"std={stats['std']:.3f} ms  min={stats['min']:.3f} ms  max={stats['max']:.3f} ms  "
    ...
    print("\n" + report)
    print(f"[Latency] Raw per-frame CSV : {LATENCY_LOG_PATH}")
    print(f"[Latency] Summary report    : {LATENCY_SUMMARY_PATH}")
    ...
    try:
        with connection.open():
            ...
            while frame_number < total_frames:
                ...
                # t_arrival = the instant the fresh frame became available to us.
                # Taken AFTER cap.read() returns so the blocking wait for the next
                # camera frame (up to ~33 ms of idle time) is NOT counted as latency.
                # This is the anchor for the end-to-end per-frame latency measurement.
                ...
                if ret and cam_frame is not None:
                    try:
                        frame_queue.put_nowait((frame_number, hand_data, cam_frame, t_arrival))
                    except Exception:
