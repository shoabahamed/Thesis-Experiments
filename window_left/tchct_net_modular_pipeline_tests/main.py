...
# --seed must take effect BEFORE config is imported: config seeds all RNGs
# at import time and every module reads SEED from it. Peek at argv here and
# hand the value to config via the THCT_SEED environment variable.
for _i, _arg in enumerate(sys.argv):
    if _arg == "--seed" and _i + 1 < len(sys.argv):
        os.environ["THCT_SEED"] = sys.argv[_i + 1]
    elif _arg.startswith("--seed="):
        os.environ["THCT_SEED"] = _arg.split("=", 1)[1]
...
def run_single_fold(
    ...
    """Run the data-split + dataloader stage for one fold and report pipeline diagnostics.

    Training/evaluation is disabled for now (pipeline inspection only) — see the
    commented-out block below for the code that used to train and evaluate the
    model. The val split is still built (for later use) but nothing evaluates
    against it while training is disabled.

    Returns a dict with pipeline-level stats for this fold (window counts, etc).
    """
    print(f"\n{'='*70}")
    print(f"FOLD: test_user={test_user}, dev_users={dev_users}")
    print(f"{'='*70}")

    # Re-seed all RNGs at fold start so results depend only on the test user,
    # not on how much random state earlier folds consumed. This makes a
    # single-fold run bit-identical to the same fold inside a LOUO sweep
    # (on the same hardware/software; cudnn determinism is set in config).
    fold_seed = SEED + sum(ord(c) for c in test_user)
    random.seed(fold_seed)
    np.random.seed(fold_seed)
    torch.manual_seed(fold_seed)
    torch.cuda.manual_seed_all(fold_seed)
    print(f"Fold seed: {fold_seed} (base {SEED} + user offset)")

    # ── 1. Split data (prints per-fold segment-drop reasons; see data_splitting.print_filter_report) ──
    ...
    # ── 2. Build dataloaders (train/val/test windows + augmentation wiring) ──
    ...
    # ── 3. Augmentation usage stats ──
    # Walk every training window once so the pipeline's internal counters
    # reflect what would happen over one full training epoch for this fold.
    print(f"\n---- Augmentation usage (test_user={test_user}) ----")
    if augment_pipeline is not None:
        for i in range(len(train_ds)):
            train_ds[i]
        print(augment_pipeline.stats_report())
    else:
        print("  Augmentation disabled for this fold.")

    # ------------------------------------------------------------------------
    # TRAINING + EVALUATION (disabled for pipeline inspection — uncomment to restore)
    # ------------------------------------------------------------------------
    ...
    #
    # # ── Streaming metrics (SHREC'21: DR, FPR, Jaccard) ──
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Base random seed for the whole run (default: 42, or THCT_SEED env var). "
            "Applied before config import; also shifts the dev train/val split "
            "(DEV_VAL_SEED derives from it). Use different seeds for variance runs."
        ),
    )
    parser.add_argument(
    ...
    try:
        ...
        # ── Final summary (pipeline diagnostics only — training/WER stats disabled) ──
        print(f"\n{'='*70}")
        ...
        # LOUO aggregate accuracy/WER stats removed along with training — see the
        # commented-out block in run_single_fold() for how this used to work.
